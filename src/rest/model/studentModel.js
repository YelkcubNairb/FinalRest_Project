...
var studentModel = new Schema({
    studentId: { type: String, index: true, unique: true }, // NOTE ilker will create an index in mongodb
    name: String,
    lastname: { type: String, trim: true, lowercase: true },
    grade: { type: String, trim: true, lowercase: true, enum: ['freshman', 'sophomore', 'junior', 'senior'] },
...
module.exports = mongoose.model("Student", studentModel); // NOTE ilker this returns a mongoose "model" and a collection called "students"(by default collection name is plural of model name, like for model name "Student", default collection name will be "students") will be created in mongodb
